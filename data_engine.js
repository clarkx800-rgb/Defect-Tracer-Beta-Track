        ...
        if (dirHandle) {
            ...
            for (let j = 0; j < pdfJobs.length; j++) {
                ...
                for(let r of job.refs) { 
                    ...
                    else if (r.img) { imgs.push({ data: r.img, w: r.w || 2400, h: r.h || 2400 }); }
                }
...
async function generatePDFBlob(job, imgs) {
    ...
    for(let i=0; i<imgs.length; i++) {
        const imgObj = imgs[i]; 
        let imgW = imgObj.w || 2400; 
        let imgH = imgObj.h || 2400; 
        ...
        const safeZoneW = pageWidth - (margin * 2); 
        const safeZoneH = pageHeight - (margin * 2); 
